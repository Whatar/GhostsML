...
## 0. Come usare il progetto

Entrare nella cartella del sorgente con:

`cd src`

Per lanciare la versione allenata dei fantasmi contro pacman, eseguire il comando:

`python ./run.py`

Per allenare nuovamente pacman:

`python ./run.py -t` o `python ./run.py --train`

Per abilitare il debug (non usa la versione allenata dei fantasmi):

`python ./run.py -d` o `python ./run.py --debug`

Per visualizzare i 4 fantasmi:

`python ./run.py -4` o `python ./run.py --4ghosts`
...
- Una transizione è codificata in questo formato: `Transition = namedtuple('Transition', ('state', 'action', 'next_state', 'reward'))` ovvero (_S, A, \_S_′, R\_). La codifica dello stato è illustrata nel capitolo [3.2]()
